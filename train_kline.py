...
def main():
    ...
    print(f"运行模式: GitHub Actions={is_github}, Colab={is_colab}, CPU Only={is_cpu_only}")

    # 加载预训练权重（从零训练，通用目标检测预训练）
    ...
    train_args = dict(
        ...
        hsv_s=0.7,
        hsv_v=0.4,
        degrees=10.0,           # 旋转增强（K线形态旋转后仍有效）
        translate=0.1,
        scale=0.5,
        flipud=0.0,             # K线不能上下翻转（时间顺序固定）
        fliplr=0.5,
        mosaic=1.0,             # 马赛克增强（最强数据增强）
        mixup=0.1,              # 混合增强
        copy_paste=0.1,
        erasing=0.4,
        crop_fraction=1.0,
    ...
    print(f"数据增强: mosaic=1.0, mixup=0.1, hsv/s/v增强已开启")
